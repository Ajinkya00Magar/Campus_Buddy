...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: event }, { data: profile }, { data: rsvp }, { data: participants }] = await Promise.all([
    supabase.from('events').select('*, users(name, avatar_url), event_participants(count)').eq('id', id).single(),
    supabase.from('users').select('*').eq('id', user.id).single(),
    supabase.from('event_participants').select('*').eq('event_id', id).eq('user_id', user.id).maybeSingle(),
    supabase.from('event_participants').select('*, users(name, avatar_url)').eq('event_id', id).eq('status', 'going').limit(20),
  ])
