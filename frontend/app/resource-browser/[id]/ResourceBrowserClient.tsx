...
export default function ResourceBrowserClient({
  resourceId,
  resourceTitle,
  provider,
  sourceUrl,
    ...
    <main className="flex min-h-screen flex-col bg-background">
      <header className="flex h-14 items-center justify-between border-b bg-card px-3 md:px-5">
        <Button asChild variant="ghost" size="sm">
          <Link href={`/${resourceId}`}>
            <ArrowLeft className="h-4 w-4" />
            Back
          </Link>
        </Button>
        <div className="min-w-0 px-3 text-center">
          <p className="truncate text-sm font-semibold text-foreground">{resourceTitle}</p>
      ...
      <section className="flex flex-1 items-center justify-center px-6 py-12 text-center">
        <div className="max-w-md">
          ...
          <h1 className="text-2xl font-bold text-foreground">Opening {resourceTitle}</h1>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Campus Buddy is switching to a full-screen in-app browser. Use the browser or WebView back control to return here.
          </p>
